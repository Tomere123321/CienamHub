...
const DeleteMember = ({ closeModal, member }) => {

  const handleDelete = async (e) => {
    e.preventDefault();
    try {
      await axios.delete(`http://localhost:8000/members/delete/${member._id}`);
      toast.success(`Member ${member.name} deleted successfully`);
      closeModal();
    } catch (error) {
      toast.error(`Failed to delete member ${member.name}. Please try again.`);
    }
  };
